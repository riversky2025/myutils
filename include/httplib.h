...
#define CPPHTTPLIB_UINT64_MAX_LENGTH (std::numeric_limits<uint64_t>::max)()
...
#define SO_OPENTYPE 0x7008
			...
			if (has_header(x.headers, "Content-Length")) {
				...
				if ((len > payload_max_length) ||
					...
					(sizeof(size_t) < sizeof(uint64_t) &&
						len > CPPHTTPLIB_PAYLOAD_MAX_LENGTH)) {
	...
	inline bool Client::process_request(Stream& strm, Request& req, Response& res,
		...
		if (req.method != "HEAD") {
			bool exceed_payload_max_length = false;
			if (!detail::read_content(strm, res, CPPHTTPLIB_UINT64_MAX_LENGTH,
